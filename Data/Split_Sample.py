...
def __movefile__ (filename,dir,lenght,flag = "",pre=True):
    ...
    for idx in range(lenght):
        width =len(list_dir_file[idx].split("/")[-1])
        number = list_dir_file[idx][-5] if width == 12 else list_dir_file[idx][-6:-4]
        file_new = list_dir_file[idx][:-5 if width == 12 else -6] + "_" + str(number) + ".npy"
        os.rename(list_dir_file[idx], file_new)
...
__movefile__("ReplayAttackSample","",-1,"",False)
